...
ただし後者（意味的判断）に**絶対的な土台は無い**。機械の縛りと違い破られうるので、MPA は確率的に締めることを目指す（P4 / [`50-enforcement.md`](./50-enforcement.md)）。
...
| `50-enforcement.md` | ルールの強制方法（多層防御 / hook / 機械検証）の思想 | 全員・運用設計 |
| `55-machine-checks.md` | L3（機械の土台）の実体：設定例 / 導入点検表 / 失敗ハンドリング / テスト戦略 | 運用設計・導入者 |
