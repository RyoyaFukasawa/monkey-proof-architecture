...
//
// → ユーザーの `npx create-mpa` はネット越し clone 不要（速い・本家 URL 固定の脆さがない）。
...
const repoRoot = resolve(here, "..", "..", ".."); // packages/create-mpa/scripts -> repo root
...
async function main() {
  for (const name of SOURCES) {
    ...
    if (!existsSync(src)) {
      throw new Error(
        `正本が見つかりません: ${src}\n` +
          `create-mpa は同一リポジトリ内の constitution/ と templates/ を取り込みます。` +
